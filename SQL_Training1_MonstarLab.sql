...
-- Cau 3
select project_name, company_name, category_name, project_spend, project_variance, revenue_recognised, count(user_id) as userCount
from `projects` p inner join `categories` ca on p.category_id = ca.id 
...
select  project_name, username, email, `password`, birthday, image_url, `role`
